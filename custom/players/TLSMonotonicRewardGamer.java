...
public class TLSMonotonicRewardGamer extends StateMachineGamer {
/*
 ...
 * can be reached. Which state is actually reached depends on what moves other players make.
 */
	...
	private Move OutOfTimeGetMove(StateMachine m, Role r, MachineState s) throws MoveDefinitionException, TransitionDefinitionException, GoalDefinitionException{
		/*
		 * Exhaustively search all the joint moves possible and take the action that maximizes the value of the next state that
		 * can be reached. Which state is actually reached depends on what moves other players make.
		 */
		...
		for (int i = 0; i < jointMoves.size(); i++) {
			...
			int temp = m.getGoal(m.findNext(move, s), r);
			if (temp == 100) {
	...
	public String getName() {
		// TODO Auto-generated method stub
		return "TLSMonotonicRewardGamer";
	}
